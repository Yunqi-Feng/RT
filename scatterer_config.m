...
switch(scatterer_dim)
    ...
    case 1
        ...
        %z_trsh=0.7*confg_dim(3);
    case 2
        x_trsh=min([8,0.5*confg_dim(1)]);
        y_trsh=min([8,0.5*confg_dim(2)]);
        ...
        % z_trsh=20;
    case 3
        x_trsh=min([5,0.5*confg_dim(1)]);
        y_trsh=min([5,0.5*confg_dim(2)]);
        z_trsh=0.7*confg_dim(3);
    otherwise
...
if z_trsh1~=z_trsh
    for i=1:scatterer_num
        if scatterer_dim~=3
            z_dep(i)=z_trsh1*(0.5+0.5*rand);
        else
            z_dep(i)=z_trsh1*rand;
        end
    end
end
% initialization
